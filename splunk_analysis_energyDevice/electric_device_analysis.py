...
def getPeaks(dataDF):
    ...
    for thisIdx, thisEvent in dataDF.iterrows():
        ...
        deltaTimeFrequency = datetime.timedelta(seconds=1)
        deltaTimeFrequency_notest= datetime.timedelta(seconds=12)
        #deltaTimeFrequency = (thisEvent['timeStamp'][-1] - thisEvent['timeStamp'][0]).total_seconds()
...
def doSplunkSearch(smartplugIdList, StartTime, EndTime):
    ...
    #check to see if I need have some rights to be able to connnect
    #service = client.connect(host='ip-172-31-16-50', port=8000, username='admin', password='changeme123')
